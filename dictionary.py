...
def pbkdf_hash(word,salt,loop):
    return hashlib.pbkdf2_hmac('sha256',str.encode(word.strip()),str.encode(salt.strip()),loop).hex()
...
def Loop_attack_pbkd (my_file_origin, dictionary_file_origin,loop):
    try:
        with open(my_file_origin, "r", encoding="latin-1") as my_file:
            for i,line in enumerate(my_file):
                ans = dictionary_attack_pbkd(line[:64],dictionary_file_origin,line[65:],loop)
                if ans != None:
                    print(f"Find at index: {i}")
    except FileNotFoundError as e:
        print(f"[ERROR] File not found: {e}")
